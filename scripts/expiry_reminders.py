#!/usr/bin/env python3
"""Напоминания о расторжении активных договоров (сообщения кладутся прямо в штатный центр уведомлений NocoBase, канал «Сроки договоров»).

Дата окончания договора больше не используется: срок задаёт необязательная «Дата расторжения договора»,
её заполняют уже у действующего договора.
...
Статус «На расторжении» ставит сама база (db/migrations/002_contract_status_auto.sql), этот скрипт только напоминает.
    ...
    expiry_reminders.py --seed     # записать в журнал как «уже отправлено» без уведомлений
...
for c in contracts:
    ...
    th = min(cands)
    key = (c['id'], th, key_date)
    if key in sent: continue
    end_txt = end.strftime('%d.%m.%Y')
    msg = 'Расторжение договора сегодня (%s)' % end_txt if days == 0 else 'До расторжения договора %d дней (%s)' % (days, end_txt)
    ...
    stmts.append("insert into contract_reminders_log(contract_id,threshold,end_date) values(%s,%s,%s);" % (c['id'], th, q(key_date)))
    if not SEED:
        for uid in rcpt:
...
for r in report: print('  договор #%s порог %s (дней до расторжения: %s), получателей %s: %s' % r)
