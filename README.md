...
- **Только непрочитанное:** вкладки «Все / Непрочитанные / Прочитано» скрыты CSS-правилом (оно лежит в начале `src/contract-modals.js`, для других страниц добавьте его в их JS-блок), а прочитанное сообщение сразу удаляется триггером БД `db/purge_read_notifications.sql` — в центре сообщений остаются только непрочитанные. Триггер живёт в PostgreSQL, а не в настройках NocoBase: при переносе на другую БД его нужно применить отдельно.
